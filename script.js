class EventHubLanding {
  ...
  setupParallaxEffects() {
    ...
    const updateParallax = () => {
      ...
      if (this.parallaxBg && scrollY < windowHeight) {
        const speed = 0.5;
        this.parallaxBg.style.transform = `translate3d(0, ${scrollY * speed}px, 0)`;
      }
      
      if (this.parallaxGlow && scrollY < windowHeight) {
        const speed = 0.3;
        this.parallaxGlow.style.transform = `translate3d(0, ${scrollY * speed}px, 0) scale(${1 + scrollY * 0.0005})`;
      }
      ...
      this.parallaxImages.forEach((image, index) => {
        ...
        if (isVisible) {
          const speed = 0.1 + (index * 0.02); // Different speeds for each image
          ...
          const rotation = (rect.top - windowHeight / 2) * 0.01;
          const imageContainer = image.querySelector('.image-container');
          if (imageContainer) {
            imageContainer.style.transform = `translateX(0) rotateY(${rotation}deg)`;
          }
        }
      });
